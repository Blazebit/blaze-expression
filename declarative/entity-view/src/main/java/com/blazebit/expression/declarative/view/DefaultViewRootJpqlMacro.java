...
public class DefaultViewRootJpqlMacro implements ViewRootJpqlMacro, CacheableJpqlMacro {
    ...
    private static final String NAME = "VIEW_ROOT";

    private final String viewRoot;

    /**
     * Creates a new view root macro.
     *
    ...
    /**
     * Registers a new view root macro with the given alias if none is present.
     *
     * @param serializer The serializer
     * @param alias The alias
     */
    public static void registerIfAbsent(PersistenceExpressionSerializer serializer, String alias) {
        if (serializer.getProperties().containsKey(NAME)) {
            return;
        }
        DefaultViewRootJpqlMacro macro = new DefaultViewRootJpqlMacro(alias);
