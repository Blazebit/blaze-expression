 ...
 * @since 1.0.0
 */
public class ExcelRoundFunction implements ExcelFunctionRenderer, Serializable {

    private static final ExcelRoundFunction INSTANCE = new ExcelRoundFunction();

    private ExcelRoundFunction() {
    }
    ...
    public static void addFunction(DomainBuilder domainBuilder) {
        domainBuilder.extendFunction("ROUND", new ExcelFunctionRendererMetadataDefinition(INSTANCE));
    }
    ...
    public void render(DomainFunction function, DomainType returnType, ExcelDomainFunctionArgumentRenderers argumentRenderers, StringBuilder sb, ExcelExpressionSerializer serializer) {
        ...
        if (argumentRenderers.assignedArguments() == 1) {
            sb.append(serializer.getArgumentSeparator()).append(" 0");
        }
        sb.append(')');
    }
}
